...
async function makeWorkspace(): Promise<{ workspace: string; configPath: string; fakeBin: string }> {
  const workspace = await fs.mkdtemp(path.join(os.tmpdir(), "auto-exp-research-brain-live-"));
...
if agent == "Apollo":
    ...
    print(json.dumps({"choice":"representation","title":"grounded-orthogonal","family":"repr.feature","innovation_tags":["representation"],"mechanism":"grounded divergence","files_to_touch":["src/strategy.txt"],"expected_gain":0.01,"risk":"medium","why_not_parameter_only":"changes representation path","minimal_ablation":["revert strategy"],"paper_grounding":[{"paper_id":"doi:10.1145/3711896.3736964","why_relevant":"support","mechanism_transfer":"semantic prompt"},{"paper_id":"paper:arxiv:2024:ffffeeee11","why_relevant":"orthogonal","mechanism_transfer":"test-time training"}]}))
elif agent == "Athena":
    print(json.dumps({"verdict":"approve","validity_risks":[],"smallest_repair":None,"single_change_ok":True,"paper_support_ok":True}))
elif agent == "sisyphus-junior":
    print(json.dumps({"touched_files":["src/config.json"],"diff_summary":"grounded change","change_manifest":{"primary_object":"objective","secondary_objects":[]}}))
else:
...
describe("research brain live grounding", () => {
  it("persists paper_grounding in the live proposal path", async () => {
